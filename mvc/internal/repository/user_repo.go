...
func (r *UserRepo) GetUserByID(id int) (*model.Users, error) {
	row := r.DB.QueryRow("select id,username,email,password,age,created_at,updated_at from users where id = ?", id)
...
func (r *UserRepo) GetAllUser() ([]model.Users, error) {
	rows, err := r.DB.Query("select id,username,email,password,age,created_at,updated_at from users")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
